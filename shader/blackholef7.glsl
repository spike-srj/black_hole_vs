...
StepResult light_step_geodesic_spherical(
    ...
    float k_r_direction_current, // 当前的 k_r 符号
    float E_const,
    float L_magnitude_const,   // 角动量大小 (替代之前的 L_const)
    float d_lambda,
    float L_z_effective        // 带符号的有效Lz
    // bool prev_step_reversed_k_r // (可选, 用于迟滞)
) {
    StepResult result;
    // result.prev_step_reversed_k_r = false; // (可选, 用于迟滞)
    ...
    float r_old = s.r; // r at the start of this RK4 step
    float next_k_r_dir_candidate = k_r_direction_current; // 开始时假设不变

    // 检查是否在视界内或非常接近奇点，这些区域的行为特殊
    if (r_new < SchwarzschildRadius * (1.0 - 1e-6)) { // 明确在视界内部
        next_k_r_dir_candidate = -1.0; // 必须向内
    } else if (r_new < 1e-5) { // 非常接近奇点 (但仍在视界外，如果Rs<1e-5)
        next_k_r_dir_candidate = -1.0; // 也应向内
    } else {
        // 在视界外部或边缘，计算 (dr/dλ)^2
        float r_sq_new = r_new * r_new;
        float V_eff_at_r_new = 0.0;
        // 确保 r_new > Rs 才能应用标准 V_eff 公式, 且 r_new 不能太小导致数值问题
        if (r_new > SchwarzschildRadius * (1.0 + 1e-7) && r_new > 1e-5 ) {
            float term_Rs_r = SchwarzschildRadius / r_new;
            float term_L_r2 = (L_magnitude_const * L_magnitude_const) / r_sq_new;
            V_eff_at_r_new = (1.0 - term_Rs_r) * term_L_r2;
        } else {
            // 恰好在视界上，或由于数值误差略小于Rs但未被上面条件捕获，或r非常小
            // 这种情况下，V_eff 的行为复杂，简单起见，如果光子还能运动，
            // 视界不是经典反弹点，所以 dr_dlambda_sq 通常不为负。
            // 但如果目标是反弹逻辑，这里可以认为势能极高，强制反弹。
            // 为简单，如果r_new非常接近Rs，我们让后续逻辑判断。
            // 如果严格在 r_new = Rs，V_eff=0，dr_sq = E^2。
            // 如果略小于Rs，V_eff < 0，dr_sq > E^2。
            // 所以，除非是特殊情况，我们主要关心 r > Rs 的转折点。
            // 如果到了这里，说明 r_new 仍然在 SchwarzschildRadius * (1.0 + 1e-7) 之外，
            // 或者恰好等于它，或者在 1e-5 和 Rs 之间。
            // 我们还是按公式计算，但要小心分母为0.
             if (r_new > 1e-5) { // 只要r不是极小
                float term_Rs_r = SchwarzschildRadius / r_new; // 可能 r_new = Rs 导致 1-Rs/r = 0
                float term_L_r2 = (L_magnitude_const * L_magnitude_const) / r_sq_new;
                V_eff_at_r_new = (1.0 - term_Rs_r) * term_L_r2;
             } else {
                V_eff_at_r_new = 1e20; // r极小，势能极大
             }
        }
        float dr_dlambda_sq_at_r_new = E_const * E_const - V_eff_at_r_new;

        const float FORBIDDEN_THRESHOLD = -1e-9;
        const float TURNING_POINT_THRESHOLD = 1e-10; // (dr/dλ)^2 接近0的阈值
        const float MIN_R_CHANGE_FOR_SIGN_UPDATE = 1e-8;

        // 引入迟滞：如果上一步刚刚反转，这一步先不要轻易再反转，除非明确进入禁止区
        // bool just_reversed_previously = currentState.prev_step_reversed_k_r; (假设这个状态被传递)

        if (dr_dlambda_sq_at_r_new < FORBIDDEN_THRESHOLD /*&& !just_reversed_previously*/) {
            next_k_r_dir_candidate = -k_r_direction_current;
            // result.prev_step_reversed_k_r = true;
        } else if (abs(dr_dlambda_sq_at_r_new) <= TURNING_POINT_THRESHOLD /*&& !just_reversed_previously*/) {
            next_k_r_dir_candidate = -k_r_direction_current;
            // result.prev_step_reversed_k_r = true;
        } else { // 在允许区域 dr_dlambda_sq > 0
            // result.prev_step_reversed_k_r = false;
            float r_change_in_step = r_new - r_old;
            if (abs(r_change_in_step) > MIN_R_CHANGE_FOR_SIGN_UPDATE) {
                next_k_r_dir_candidate = (r_change_in_step > 0.0) ? 1.0 : -1.0;
            } else {
                // r 变化很小，但不在转折点。保持当前方向。
                // 如果 k_r_direction_current 是 0，这里需要特殊处理来“启动”它。
                // 但通常 k_r_direction_current 应该是 +1 或 -1 （如果 L > 0）。
                // 如果它因为某种原因变成0了，而这里 dr_dlambda_sq > 0，
                // 说明可以运动。默认可以尝试向外，或基于 dV_eff/dr。
                // 为简单，如果它是0，我们让它保持0，直到某个条件强制它变为+1或-1。
                // 或者，如果它是0，且可以动，则基于之前的趋势（如果能记录）或默认向外。
                if (abs(k_r_direction_current) < 0.5 && L_magnitude_const > EPSILON && dr_dlambda_sq_at_r_new > TURNING_POINT_THRESHOLD) {
                    // 之前静止，现在可以动，但r变化不大。
                    // 这个分支逻辑比较微妙，可能需要更多上下文。
                    // 例如，如果它在光子球层附近，它可能需要根据微小扰动决定方向。
                    // 简单起见，如果之前是0，现在可以动，我们先不改变它，等r_change_in_step变大。
                    // 或者，如果我们知道它刚从一个转折点反弹出来，current_k_r_direction 已经是新的方向了。
                }
                // else next_k_r_dir_candidate 保持 current_k_r_direction
            }
        }
    }

    // 最终的视界检查和强制 (覆盖之前的candidate)
    if (r_new < SchwarzschildRadius * (1.0 - 1e-7)) { // 明确在视界内部
        result.next_k_r_direction = -1.0;
    } else if (r_new < SchwarzschildRadius * (1.0 + 1e-7) && next_k_r_dir_candidate > 0.5) { // 非常接近视界但想向外
        result.next_k_r_direction = -1.0; // 强制向内
    } else {
        result.next_k_r_direction = next_k_r_dir_candidate;
    }
...
//            }
        ...
        if(k_r_direction != step_res.next_k_r_direction)
        {
            dirChangeCout++;
        }
    ...
    // --- 4. 确定最终颜色 ---
    if (alpha <= 0) { // 或者 if (currentState.r <= SchwarzschildRadius)
...
//        FragColor = vec4(dirChangeCout/2000, dirChangeCout,dirChangeCout, 1.0); // 灰度显示，问题出在LL/RR上
//        FragColor = vec4(currentState.r,1,1, 1.0); // 灰度显示，问题出在LL/RR上
