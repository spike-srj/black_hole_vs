...
float R_FAR_FROM_BH = SchwarzschildRadius * 50.0; // 定义一个“远离黑洞”的区域
...
StepResult light_step_geodesic_spherical(
    ...
    // k2
    PhotonStateSpherical deriv2 = get_schwarzschild_geodesic_derivatives(s.r + 0.5 * d_lambda * dr1, s.theta + 0.5 * d_lambda * dt1, E_const, L_const,L_z_effective);
    ...
    // k3
    PhotonStateSpherical deriv3 = get_schwarzschild_geodesic_derivatives(s.r + 0.5 * d_lambda * dr2, s.theta + 0.5 * d_lambda * dt2, E_const, L_const,L_z_effective);
    ...
    // k4
    PhotonStateSpherical deriv4 = get_schwarzschild_geodesic_derivatives(s.r + d_lambda * dr3, s.theta + d_lambda * dt3, E_const, L_const,L_z_effective);
...
//    return result;
    ...
    float next_k_r_direction = k_r_direction_current; // 默认保持不变
    // 1. 计算在新位置 r_new 的 (dr/dλ)^2
    float r_sq_new = r_new * r_new;
    float dr_dlambda_sq_at_r_new;
    if (r_new <= SchwarzschildRadius * (1.0 + 1e-5) || r_new < 1e-5) { // 接近或在视界内/奇点
        // 在视界内，物理上光子只能向内。如果之前向外（不可能）或在视界边缘，强制向内或停止。
        // 简单处理：如果已经掉入很深，径向速度可以认为是负的或0。
        // 视界处的 V_eff(Rs) = 0，所以 E^2 - V_eff(Rs) = E^2 > 0，这表示视界不是一个经典转折点。
        // 一旦进入视界，光子必然落向r=0。
        // 如果数值上 r_new 略小于 Rs，但光子本应从外部接近，
        // 它的 k_r_direction 应该已经是 -1.0。
        // 这里主要是防止由于数值误差“跳出”视界后错误地将方向设为+1。
        if (r_new < SchwarzschildRadius * (1.0 - 1e-7) && k_r_direction_current > 0.5) { // 深度进入且之前向外
             next_k_r_direction = -1.0; // 强制向内
        } else if (r_new < SchwarzschildRadius * (1.0 - 1e-7) && abs(k_r_direction_current) < 0.5) { // 深度进入且之前静止
             next_k_r_direction = -1.0; // 强制向内
        }
        // 如果只是非常接近 Rs 但仍在外面，或者刚穿过，则按正常逻辑判断。
        // 对于 dr_dlambda_sq_at_r_new 的计算：
        // 如果 r_new <= Rs，(1-Rs/r_new) <= 0。若 L > 0, V_eff <= 0。
        // E^2 - V_eff >= E^2。所以 dr_dlambda_sq 总是正的。
        // 这意味着视界本身不是一个“反弹”的转折点。
        // 我们主要关心外部的转折点。
        // 为简化，如果已在视界内，不再严格计算V_eff，因为那里的物理不同。
        // 此时主要依赖之前的k_r_direction或视界吸收逻辑。
        // 如果在视界外，则正常计算：
        if (r_new > SchwarzschildRadius * (1.0 + 1e-5) && r_new > 1e-5) { // 明确在视界外且远离奇点
            float term_Rs_r = SchwarzschildRadius / r_new;
            float term_L_r2 = (L_const * L_const) / r_sq_new;
            float V_eff_at_r_new = (1.0 - term_Rs_r) * term_L_r2;
            dr_dlambda_sq_at_r_new = E_const * E_const - V_eff_at_r_new;
        } else {
            // 在视界内或非常接近，或接近奇点。
            // 如果严格按视界内物理，应该总是向r=0运动。
            // 为了转折点逻辑的完整性，如果进入此分支，可以认为 dr_dlambda_sq 很大（允许运动）
            // 但 k_r_direction 最终会被视界逻辑覆盖。
            // 或者，如果目标是检测外部转折点，这里可以设置一个特殊值表明已过视界。
            dr_dlambda_sq_at_r_new = E_const * E_const; // 假设在视界内径向运动不受L阻碍
                                                      // (这只是为了让下面的逻辑有值，实际应由视界逻辑处理)
        }
    } else { // 在视界之外的正常区域
        float term_Rs_r = SchwarzschildRadius / r_new;
        float term_L_r2 = (L_const * L_const) / r_sq_new;
    ...
    // 2. 定义阈值
    const float FORBIDDEN_THRESHOLD = -1e-9; // (dr/dλ)^2 显著为负的阈值
    const float TURNING_POINT_THRESHOLD = 1e-10; // (dr/dλ)^2 接近0的阈值
    const float MIN_R_CHANGE_FOR_SIGN_UPDATE = 1e-8; // r 实际变化的最小量
    // 3. 更新 k_r_direction
    if (dr_dlambda_sq_at_r_new < FORBIDDEN_THRESHOLD) {
        // **情况1: 数值上进入了禁止区域 (E² << V_eff(r_new))**
        // 这通常意味着RK4步长过大，"冲过"了真正的转折点。
        // 物理上光子应该已经反弹。
        next_k_r_direction = -k_r_direction_current; // 强制反转方向
        // 注意：此时 r_new 可能在物理不允许的区域。一个更复杂的处理方法是
        // 将 r_new 移回到 E² = V_eff(r) 的边界，或者减小步长重新计算这一步。
        // 但对于实时渲染，简单反转方向然后继续可能是可接受的。
        if (abs(k_r_direction_current) < 0.5 && L_const > EPSILON) { // 如果之前是静止的(k_r=0), 且有角动量
            // 需要判断应该朝哪个方向反弹。这通常需要知道是从哪个方向接近这个“禁止区”的。
            // 如果没有历史信息，一个简单的策略是尝试向外，因为通常是从外向内遇到势垒。
            // 但这很棘手。更好的做法是 k_r_direction_current 不应该为0除非L=0。
            // 暂时假设 k_r_direction_current 总是 +1 或 -1 (如果 L > 0)。
            // 如果 k_r_direction_current 为0，通常意味着 L=0，那么 V_eff=0 (或负)，不会进入此分支。
        }
    } else if (abs(dr_dlambda_sq_at_r_new) <= TURNING_POINT_THRESHOLD) {
        // **情况2: 到达或非常接近转折点 (E² ≈ V_eff(r_new))**
        // 径向速度几乎为零。光子应该反向运动。
        next_k_r_direction = -k_r_direction_current;
        // 与情况1类似，如果 k_r_direction_current 是0，如何决定反弹方向比较复杂。
        // 再次假设 k_r_direction_current 是 +1 或 -1。
        // 如果多次停留在这个状态，可能需要特殊处理（例如，轨道非常接近光子球层）。
    } else {
        // **情况3: 仍在允许区域且 (dr/dλ)² > 0**
        // 此时，最可靠的是看 r 在这一步的实际变化方向。
        float r_change_in_step = r_new - currentState.r; // r_old是本RK4步骤开始时的r
        if (abs(r_change_in_step) > MIN_R_CHANGE_FOR_SIGN_UPDATE) {
            next_k_r_direction = (r_change_in_step > 0.0) ? 1.0 : -1.0;
        } else {
            // r 几乎没有变化，但我们不在转折点附近，且 (dr/dλ)² > 0。
            // 这可能是因为：
            // a) 主要是切向运动，径向速度很小但非零。
            // b) RK4步长 d_lambda 太小，导致 r 的变化小于 MIN_R_CHANGE_FOR_SIGN_UPDATE。
            // c) 数值积分在某个局部极小/极大势能附近“卡住”了（不常见于光子逃逸）。
            // 在这种情况下，保持 k_r_direction_current 通常是安全的，因为它代表了之前的趋势。
            // 如果 k_r_direction_current 是0，而 (dr/dλ)² > 0，这说明光子应该开始径向运动了。
            // 此时，需要一个更复杂的逻辑来决定初始方向 (例如，根据dV_eff/dr的符号，
            // 如果 dV_eff/dr < 0，势能向内减小，光子倾向于向内；反之向外)。
            // 但对于光线追踪，通常 k_r_direction_current 在L>0时不会是0，除非精确在转折点。
            // 如果确实发生了 k_r_direction_current = 0 且 (dr/dλ)² > 0，
            // 一个可能的默认是尝试向外 (+1.0)，或者基于之前的非零k_r_direction。
            // 简单起见，如果r变化不大，我们暂且保持之前的非零k_r_direction。
            // 如果之前的k_r_direction是0，这里需要一个打破僵局的策略。
            if (abs(k_r_direction_current) < 0.5 && L_const > EPSILON) {
                // 之前径向静止，但现在可以动了，且有角动量。
                // 这通常发生在从一个精确的转折点开始的下一步。
                // 它的方向应该已经是上一步反转后的方向了。
                // 所以这里保持 next_k_r_direction = k_r_direction_current (即0) 可能不理想。
                // 但如果上一步已经正确设置了 k_r_direction_current (例如从 -1 变为 +1)，
                // 那么这里保持它是对的。
                // 这个else分支的逻辑在现实中可能较少触发关键决策。
            }
        }
    }
    // 4. 最终检查：确保在视界内的行为符合物理
    // (这段逻辑可以放在最前面或最后作为强制约束)
    if (r_new < SchwarzschildRadius * (1.0 - 1e-6)) { // 明确在视界内部
        next_k_r_direction = -1.0; // 任何在视界内的东西都必须朝向 r=0 运动
    } else if (r_new < SchwarzschildRadius * (1.0 + 1e-6) && next_k_r_direction > 0.5) {
        // 非常接近视界（或由于数值误差略微“跳出”），但方向却是向外，这不物理
        // next_k_r_direction = -1.0; // 强制向内或至少不向外
        // 或者 next_k_r_direction = 0.0; 表示在视界处被“卡住”（虽然物理上会继续掉落）
        // 更安全的做法是，如果非常接近视界，且之前是向内的，则保持向内。
        // 如果非常接近视界，且之前是向外的（理论上不应发生），则强制向内。
        // 这里的处理取决于你希望模拟的精度和行为。
        // 对于逃逸光线，一旦非常接近或进入视界，通常就判定为被捕获。
    }
    result.next_k_r_direction = next_k_r_direction;
    return result;
...
void main()
{
    ...
    if (L_const > EPSILON) {
        ...
        // 可选的稳定化：例如，尝试让 orbit_normal_w_for_basis.y > 0 (如果它不接近0)
        // 这样做的目的是为了让 v_plane_w 的方向（phi增加的方向）有一个更固定的约定
        // if (orbit_normal_w_for_basis.y < -EPSILON * 0.1) { // 阈值可以调整
        //     orbit_normal_w_for_basis *= -1.0;
        // } else if (abs(orbit_normal_w_for_basis.y) < EPSILON * 0.1) {
        //     if (orbit_normal_w_for_basis.z < -EPSILON * 0.1) {
        //         orbit_normal_w_for_basis *= -1.0;
        //     } else if (abs(orbit_normal_w_for_basis.z) < EPSILON * 0.1) {
        //         if (orbit_normal_w_for_basis.x < -EPSILON * 0.1) {
        //             orbit_normal_w_for_basis *= -1.0;
        //         }
        //     }
        // }
...
//            }
    ...
    if (alpha <= 0.0) { // 或者 if (currentState.r <= SchwarzschildRadius)
        FragColor = vec4(1.0, 0.0, 0.0, 1.0); // 黑洞
    } 
    else {
...
//        float term_L_r_potential = (L_const * L_const / r2) * (1.0 - SchwarzschildRadius / currentState.r);
//        float dr_dlambda_sq = E_const * E_const - term_L_r_potential;//term_L_r_potential不够大会导致明显的分界面
//        float dr_dlambda;
//        if (dr_dlambda_sq < 0.0) {
//        dr_dlambda = 0.0; // 光子无法到达此半径（经典转折点）
//        } else {
//            dr_dlambda = sqrt(dr_dlambda_sq);
//            // dr/dλ 的符号需要在 RK4 步进中根据当前是向内还是向外运动来确定
//        }
//        float L_sq = L_const * L_const;
//        float minVal = 0.0;    // 原始最小值
//        float maxVal = 9.0;  // 原始最大值
//        // atan(L_sq) 会将 [0, inf) 映射到 [0, PI/2)
//        // (2.0 / PI) * atan(L_sq) 会将结果映射到 [0, 1)
//        float normalized_L_sq = L_sq/maxVal;
//        if(normalized_L_sq >= 1)
//        {
//            normalized_L_sq=0;
//        }
//        dr_dlambda *= k_r_direction*dr_dlambda;
//        L_const = clamp(L_const, 0.0, 1.0);
//        FragColor = vec4(L_const * L_const / r2, L_const * L_const / r2,L_const * L_const / r2, 1.0); // 灰度显示，问题出在LL/RR上
//        FragColor = vec4(dr_dlambda_sq, dr_dlambda_sq,dr_dlambda_sq, 1.0); // 灰度显示，问题出在LL/RR上
//        FragColor = vec4(L_const * L_const / r2, L_const * L_const / r2,L_const * L_const / r2, 1.0); // 灰度显示，问题出在LL/RR上

//        float viz_dphi_dl = final_dphi_dl; // final_dphi_dl 现在应该是带符号的
//        // 尝试估计一个范围，例如 [-max_abs_dphi, max_abs_dphi]
//        // 这个 max_abs_dphi 需要你根据场景参数试验性地调整，或者从 L 和 r 的典型值估算
//        float max_abs_dphi_estimate = 2.0; // !! 重要：这个值需要根据你的具体情况调整 !!
//                                           // 如果 L_z_effective 很小，或者 r 很大，这个值可以小一些
//                                           // 如果 L_z_effective 很大，或者 r 很小，这个值需要大一些
//        viz_dphi_dl = (viz_dphi_dl / max_abs_dphi_estimate) * 0.5 + 0.5; // 映射到 [0,1]
//        viz_dphi_dl = clamp(viz_dphi_dl, 0.0, 1.0);
//        FragColor = vec4(viz_dphi_dl, viz_dphi_dl, viz_dphi_dl, 1.0); // 灰度显示 

//        float E_const_val = 1.0; // 假设你的 E_const 是 1.0
//        // 将 final_dr_dl 从 [-E_const_val, E_const_val] 映射到 [0, 1]
//        float viz_dr_dl = (final_dr_dl / E_const_val) * 0.5 + 0.5;
//        viz_dr_dl = clamp(viz_dr_dl, 0.0, 1.0); // 确保在范围内
//        FragColor = vec4(viz_dr_dl, viz_dr_dl, viz_dr_dl, 1.0); // 灰度显示 

//        if (k_r_direction > 0.5) FragColor = vec4(1.0, 0.5, 0.5, 1.0); // 偏红
//        else if (k_r_direction < -0.5) FragColor = vec4(0.5, 1.0, 0.5, 1.0); // 偏绿
//        else FragColor = vec4(0.5, 0.5, 1.0, 1.0); // 偏蓝
