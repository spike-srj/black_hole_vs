...
float R_CLOSE_TO_BH = SchwarzschildRadius * 1.6; // 定义一个“靠近黑洞”的区域，例如3倍史瓦西半径
float R_FAR_FROM_BH = SchwarzschildRadius * 5.0; // 定义一个“远离黑洞”的区域
float D_LAMBDA_BASE = D_LAMBDA;

const int NUM_SUB_RAYS = 1; // 每个像素发射的光线数量 (可以设为 uniform 从 C++ 控制)
...
//        if (final_derivs_check.dr_dlambda <= 0.01 && deriv1.dr_dlambda != 0.0) { // 刚到达转折点
//            result.next_k_r_direction = -k_r_direction_current; // 尝试反转
//        }
//    }
    if(abs(dr_total_step) < 1e-32)
    {
...
//    FragColor = vec4(accumulated_color / float(NUM_SUB_RAYS), 1.0);
    FragColor = vec4(dirChangeCout/1000,dirChangeCout/1000,dirChangeCout/1000, 1.0);
}
