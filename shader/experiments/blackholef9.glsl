...
const int   MAX_REFINES = 10;       // More backtracking refinements near critical regions
...
const float D_LAMBDA_MAX = 0.15;    // Lower max step to reduce overshoot artifacts
const float MAX_DR_FRACTION = 0.01; // Tighter radial change per step
const float MAX_DPHI = 0.03;        // Tighter angular change near photon sphere
...
void main()
{
    ...
    for (int j = 0; j < MAX_STEPS; j++) {
        ...
        if (dr_sq_at_new < 0.0) {
            // keep angular evolution but don't step radially into invalid region
            currentState.phi = wrap_angle(step_res.next_state.phi);
            // retain previous r to stay valid; k_r_direction may have flipped above
            // continue to next iteration
        } else {
            currentState = step_res.next_state;
            currentState.phi = wrap_angle(currentState.phi);
        }
        currentState.phi = wrap_angle(currentState.phi);
    }
