...
const float HORIZON_TOL = 1.0001;   // r <= Rs*HORIZON_TOL considered inside horizon; tighter to avoid false hits
const int   MAX_REFINES = 3;       // More backtracking refinements near critical regions
const float FLIP_HYST_FRAC = 0.05;  // Radial hysteresis fraction to avoid flip ping-pong
...
const float D_LAMBDA_MAX = 0.10;    // Lower max step to reduce overshoot artifacts
const float MAX_DR_FRACTION = 0.005; // Tighter radial change per step
const float MAX_DPHI = 0.02;        // Tighter angular change near photon sphere
const float PHI_SPARSE_WRAP_THRESHOLD = 500.0;  // Sparse angle wrap threshold (radians)  // Sparse angle wrap threshold (radians)
...
// Keep angle within [-PI, PI] to improve sin/cos numerical stability
...
void sparse_wrap_angle(inout float a) {
    if (abs(a) > PHI_SPARSE_WRAP_THRESHOLD) {
        a = wrap_angle(a);
    }
}
...
vec2 get_geodesic_derivatives(float r, float E, float L) {
    // Avoid singularities; compute normally near horizon to keep integration smooth.
    if (r < EPSILON) {
        return vec2(0.0);
    }
...
vec3 getFinalDirection(
    ...
    float final_dr_dl = final_derivs.x; // use outward radial at escape (decouple from k_r_direction)
    ...
    float phi = wrap_angle(final_state.phi);
    float sin_phi = sin(phi);
    float cos_phi = cos(phi);

    // Build orthonormal polar basis in the orbital plane
    vec3 r_hat     =  cos_phi * u_plane_w + sin_phi * v_plane_w;  // radial unit
    vec3 theta_hat = -sin_phi * u_plane_w + cos_phi * v_plane_w;  // tangential unit

    // Compose velocity in plane: v = (dr) r_hat + (r dφ) theta_hat
    float vr = final_dr_dl;
    float vt = final_state.r * final_dphi_dl;
    vec3 dir = vr * r_hat + vt * theta_hat;

    // Fallbacks to avoid tiny-length vectors causing discontinuities/sparkles
    float len_dir = length(dir);
    if (len_dir < 1e-12 || any_bad(dir)) {
        // Prefer the dominant component if available
        if (abs(vt) > abs(vr)) {
            dir = sign(vt) * theta_hat;
        } else if (abs(vr) > 0.0) {
            dir = sign(vr) * r_hat;
        } else {
            // Last resort: keep continuity using initial direction
            return initial_ray_dir;
        }
    }
...
void main()
{
    ...
    // Initial radial direction: force to ±1 to avoid a zero sign (tangential case).
    // dot > 0 → moving outward (r increases), dot < 0 → moving inward (r decreases).
    float k_r_direction = (dot(ray_dir_world, p_rel_initial_cart) >= 0.0) ? 1.0 : -1.0;
    ...
    for (int j = 0; j < MAX_STEPS; j++) {
        ...
        if (currentState.r <= SchwarzschildRadius * HORIZON_TOL) {
            // Only classify as captured if currently moving inward.
            if (k_r_direction < 0.0) {
                hit_horizon = true;
                break;
            }
            // Otherwise allow integration to continue (outward-moving near horizon).
        }
        ...
        // Turning-point and direction flip logic with hysteresis (stabilized)
        float r_old = currentState.r;
        float r_new_final = step_res.next_state.r;
        float r_eps = R_EPS_FRAC * max(r_old, SchwarzschildRadius);
        bool expect_outward = (k_r_direction > 0.0);
        bool moved_outward = (r_new_final > r_old + r_eps); // significance threshold
        // Compute dr^2 at new position and detect true turning via |dr/dλ|
        float dr_sq_at_new = E_const * E_const - effective_potential(max(r_new_final, SchwarzschildRadius * 1.000001), L_const);
        float dr_abs_new = sqrt(max(dr_sq_at_new, 0.0));
        bool near_turning = (dr_abs_new <= TURNING_EPS_DR);

        // Apply hysteresis: avoid flipping again until moved away sufficiently
        bool allow_flip = true;
        if (last_flip_r > 0.0) {
            float need = max(FLIP_HYST_FRAC * safe_max_r(r_new_final), FLIP_HYST_ABS);
            allow_flip = abs(r_new_final - last_flip_r) > need;
        }

        // Only flip near a true turning point (derivative-based)
        if (allow_flip && near_turning) {
        ...
        // If step ended in a forbidden region (dr^2 < 0), clamp radial move to avoid noise
        if (dr_sq_at_new < 0.0) {
            // Non-physical state detected: step crossed into forbidden region.
            // Stabilize by flipping radial direction and reverting radius, while keeping smooth angular evolution.
            if (allow_flip) {
                k_r_direction = -k_r_direction;
                last_flip_r = r_old;
            }
            currentState.r = r_old;
            currentState.phi = step_res.next_state.phi;
            sparse_wrap_angle(currentState.phi);
            continue;
        } else {
...
// SYNC_TEST 2025-08-25 22:34:21
// SYNC_TEST_2 2025-08-25 22:35:37
// SYNC_TEST_3 2025-08-25 22:36:16
