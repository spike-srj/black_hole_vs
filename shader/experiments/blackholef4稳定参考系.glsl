...
void main()
{
    ...
    vec3 n_initial_w;
    if (length(L_vec_world) > EPSILON) {
        n_initial_w = normalize(L_vec_world);
        // 可选：进一步稳定化 n_initial_w 的朝向，例如总是使其Y分量为正。
        // 这个稳定化很重要，因为它将定义 L_z_signed 的符号约定。
        if (n_initial_w.y < -EPSILON) { // 如果Y分量显著为负
            n_initial_w *= -1.0;
        } else if (abs(n_initial_w.y) < EPSILON) { // Y分量接近0
            if (n_initial_w.z < -EPSILON) { // 尝试Z分量
                n_initial_w *= -1.0;
            } else if (abs(n_initial_w.z) < EPSILON) { // Y,Z都接近0
                if (n_initial_w.x < -EPSILON) { // 尝试X分量
                    n_initial_w *= -1.0;
                }
            }
        }
    } else { // L = 0, 纯径向
        // 需要一个与 p_rel_initial_cart 不平行的法线作为参考
        // 例如，尝试与世界Y轴正交于 p_rel_initial_cart 的方向
        if (length(p_rel_initial_cart) > EPSILON) {
            vec3 temp_up = vec3(0.0, 1.0, 0.0);
            if (abs(dot(normalize(p_rel_initial_cart), temp_up)) > 0.99) { // 如果p_rel与Y平行
                temp_up = vec3(1.0, 0.0, 0.0); // 改用X轴
            }
            n_initial_w = normalize(cross(normalize(p_rel_initial_cart), temp_up));
        } else { // p_rel_initial_cart 也是0 (初始在黑洞中心)
            n_initial_w = vec3(0.0, 0.0, 1.0); // 任意选择，例如世界Z轴
        }
    }

    vec3 u_initial_w;
    if (length(p_rel_initial_cart) > EPSILON) {
        // 将p_rel_initial_cart投影到由n_initial_w定义的平面上
        u_initial_w = normalize(p_rel_initial_cart - dot(p_rel_initial_cart, n_initial_w) * n_initial_w);
        // 如果投影长度过小（理论上不应发生，除非p_rel_initial_cart平行于n_initial_w，这意味着L_vec_world计算有问题）
        // 或者 p_rel_initial_cart 本身就很短
        if(length(u_initial_w) < EPSILON) {
             // Fallback for u_initial_w (should be rare if L_vec_world & n_initial_w are well-defined)
            vec3 helper_u = vec3(1.0,0.0,0.0);
            if(abs(dot(n_initial_w,helper_u)) > 0.99) helper_u = vec3(0.0,1.0,0.0);
            u_initial_w = normalize(cross(helper_u, n_initial_w));
        }
    } else { // 初始在黑洞中心
        u_initial_w = normalize(world_dir_initial); // 初始径向就是初始方向
    }
    vec3 v_initial_w = cross(n_initial_w, u_initial_w);
    float dr_dlambda_initial_signed = dot(world_dir_initial, u_initial_w);
    float dphi_dlambda_initial_signed_times_r = dot(world_dir_initial, v_initial_w);
    float dphi_dlambda_initial_signed = 0.0;
    if (currentState.r > EPSILON) { dphi_dlambda_initial_signed = dphi_dlambda_initial_signed_times_r / currentState.r; }
    // --- 让我们用更直接的方法设置初始导数的符号 ---
    // (dr/dλ) 的初始符号：如果光线射向黑洞中心，为负。dot是cos
    float k_r_direction  = (dr_dlambda_initial_signed >= 0.0) ? 1.0 : -1.0;
    if (abs(dr_dlambda_initial_signed) < EPSILON) k_r_direction = 0.0;
    float L_z_effective = dot(L_vec_world, n_initial_w);
    // --- 3. 光线积分循环 ---
...
//            }
        ...
        // 执行 RK4 步进
        StepResult step_res = light_step_geodesic_spherical(currentState, k_r_direction, E_const, L_const, D_LAMBDA,L_z_effective);
    ...
    if (alpha <= 0.0) { // 或者 if (currentState.r <= SchwarzschildRadius)
        ...
        float final_dphi_dl = L_z_effective*final_derivs_mag.dphi_dlambda;
...
//            k_orbit_plane_cart2D.y = dot(world_dir_initial, u_initial_w);
//            k_orbit_plane_cart2D.x = dot(world_dir_initial, v_initial_w);
            // 将 k_orbit_plane_cart2D (它在 u_plane_w, v_plane_w 定义的平面内) 转换回世界坐标
            final_escape_dir_world_calculated = normalize(
                k_orbit_plane_cart2D.x * u_initial_w +  // 分量沿 u_plane_w
                k_orbit_plane_cart2D.y * v_initial_w    // 分量沿 v_plane_w
            );
        }
